...
- [Development guidelines](#development-guidelines)
    - [General ISLAMIC DIGITAL COIN](#general-islamic_digital_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ISLAMIC DIGITAL COIN uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
ISLAMIC DIGITAL COIN is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_islamic_digital_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_islamic_digital_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/islamic_digital_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_islamic_digital_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates IDCs (if wallet is enabled).
...
However, in open source software such as ISLAMIC DIGITAL COIN, where everyone uses
...
pay attention to for reviewers of ISLAMIC DIGITAL COIN code.

General ISLAMIC DIGITAL COIN
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ISLAMIC DIGITAL COIN uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ISLAMIC DIGITAL COIN so that they can be integrated
...
file descriptors it will cause problems with ISLAMIC_DIGITAL_COIN's `select()` loop, because
...
```bash
$ lsof -p $(pidof islamic_digital_coind) |\
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `islamic_digital_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `islamic_digital_coin-cli` and the GUI debug console use this table to determine how to
