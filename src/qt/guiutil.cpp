...
#define URI_SCHEME "islamic_digital_coin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter ISLAMIC_DIGITAL_COIN address (e.g. %1)").arg("igFpRWdmaJyJ2QsVzoKnyFebGjcgbX24RR"));
...
{
    // return if URI is not valid or is no ISLAMIC_DIGITAL_COIN: URI
...
{
    // Convert islamic_digital_coin:// to islamic_digital_coin:
    //
    //    Cannot handle this later, because islamic_digital_coin:// will cause Qt to see the part after // as host,
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ISLAMIC_DIGITAL_COIN (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ISLAMIC_DIGITAL_COIN (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "ISLAMIC_DIGITAL_COIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ISLAMIC_DIGITAL_COIN*.lnk
...
{
    return GetAutostartDir() / "islamic_digital_coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a islamic_digital_coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=ISLAMIC_DIGITAL_COIN (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=ISLAMIC_DIGITAL_COIN (regtest)\n";
        else
            optionFile << "Name=ISLAMIC_DIGITAL_COIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the islamic_digital_coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add islamic_digital_coin app to startup item list
