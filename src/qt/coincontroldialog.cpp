...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select IDC Outputs to Spend" :
            "Select Shielded IDC to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show ISLAMIC_DIGITAL_COIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
