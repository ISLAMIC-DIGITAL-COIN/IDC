...
#include "qt/islamic_digital_coin/islamic_digital_coingui.h"
...
#include "qt/islamic_digital_coin/splash.h"
#include "qt/islamic_digital_coin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("islamic digital coin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in islamic_digital_coin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in islamic_digital_coin.qrc)
...
/** Class encapsulating ISLAMIC DIGITAL COIN startup and shutdown.
...
/** Main ISLAMIC_DIGITAL_COIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (ISLAMIC_DIGITAL_COINGUI)
...
#include "islamic_digital_coin.moc"
...
{
    window = new ISLAMIC_DIGITAL_COINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ISLAMIC_DIGITAL_COINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ISLAMIC_DIGITAL_COINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ISLAMIC_DIGITAL_COINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ISLAMIC_DIGITAL_COINGUI::DEFAULT_WALLET);
        ...
        // ISLAMIC_DIGITAL_COIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ISLAMIC_DIGITAL_COINGUI::handlePaymentRequest);
        connect(window, &ISLAMIC_DIGITAL_COINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. ISLAMIC DIGITAL COIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(islamic_digital_coin_locale);
    Q_INIT_RESOURCE(islamic_digital_coin);
    ...
    /// 6. Determine availability of data directory and parse islamic_digital_coin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("ISLAMIC DIGITAL COIN"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("ISLAMIC DIGITAL COIN"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // islamic_digital_coin: links repeatedly have their payment requests routed to this process:
