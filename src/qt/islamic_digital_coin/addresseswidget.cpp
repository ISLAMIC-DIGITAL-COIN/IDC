...
#include "qt/islamic_digital_coin/forms/ui_addresseswidget.h"
#include "qt/islamic_digital_coin/addresslabelrow.h"
#include "qt/islamic_digital_coin/addnewaddressdialog.h"
#include "qt/islamic_digital_coin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(ISLAMIC_DIGITAL_COINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto idcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(idcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(idcAdd);
        if (walletModel->updateAddressBookLabels(idcAdd, label.toUtf8().constData(),
