...
#include "qt/islamic_digital_coin/forms/ui_send.h"
#include "qt/islamic_digital_coin/addnewcontactdialog.h"
#include "qt/islamic_digital_coin/qtutils.h"
#include "qt/islamic_digital_coin/sendchangeaddressdialog.h"
#include "qt/islamic_digital_coin/optionbutton.h"
#include "qt/islamic_digital_coin/sendconfirmdialog.h"
#include "qt/islamic_digital_coin/guitransactionsutils.h"
#include "qt/islamic_digital_coin/loadingdialog.h"
...
SendWidget::SendWidget(ISLAMIC_DIGITAL_COINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-idc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onIDCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onIDCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent IDCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield IDCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent IDCs to shield."));
    }
}
...
void SendWidget::onIDCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto idcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(idcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(idcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(idcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
