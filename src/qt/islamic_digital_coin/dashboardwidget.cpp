...
#include "qt/islamic_digital_coin/forms/ui_dashboardwidget.h"
#include "qt/islamic_digital_coin/qtutils.h"
#include "qt/islamic_digital_coin/sendconfirmdialog.h"
#include "qt/islamic_digital_coin/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(ISLAMIC_DIGITAL_COINGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareIdc, "square-chart-idc");
    setCssProperty(ui->labelIdc, "text-chart-idc");
    ...
    setCssProperty(ui->labelAmountIdc, "text-stake-idc-disable");
    ...
    if (window)
        connect(window, &ISLAMIC_DIGITAL_COINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair IDC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            idc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalIdc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesIdc.append(idc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesIdc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalIdc > 0) {
        setCssProperty(ui->labelAmountIdc, "text-stake-idc");
    } else {
        setCssProperty(ui->labelAmountIdc, "text-stake-idc-disable");
    }
    forceUpdateStyle({ui->labelAmountIdc});
    ui->labelAmountIdc->setText(GUIUtil::formatBalance(chartData->totalIdc, nDisplayUnit));
