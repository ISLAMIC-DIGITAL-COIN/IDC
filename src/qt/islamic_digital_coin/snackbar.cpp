...
#include "qt/islamic_digital_coin/forms/ui_snackbar.h"
#include "qt/islamic_digital_coin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ISLAMIC_DIGITAL_COINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ISLAMIC_DIGITAL_COINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
