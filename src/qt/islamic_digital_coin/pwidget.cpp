...
PWidget::PWidget(ISLAMIC_DIGITAL_COINGUI* _window, QWidget *parent) : QWidget((parent) ? parent : _window), window(_window) { init(); }
PWidget::PWidget(PWidget* parent) : QWidget(parent), window(parent->getWindow()) { init(); }

void PWidget::init()
{
    if (window)
        connect(window, &ISLAMIC_DIGITAL_COINGUI::themeChanged, this, &PWidget::onChangeTheme);
}
