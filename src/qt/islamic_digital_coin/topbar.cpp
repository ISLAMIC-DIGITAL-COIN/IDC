...
#include "qt/islamic_digital_coin/forms/ui_topbar.h"
#include "qt/islamic_digital_coin/lockunlock.h"
#include "qt/islamic_digital_coin/qtutils.h"
#include "qt/islamic_digital_coin/receivedialog.h"
#include "qt/islamic_digital_coin/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/islamic_digital_coin/balancebubble.h"
...
TopBar::TopBar(ISLAMIC_DIGITAL_COINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopIdc, ui->labelAmountTopShieldedIdc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountIdc}, "amount-topbar");
    setCssProperty({ui->labelPendingIdc, ui->labelImmatureIdc}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("ISLAMIC_DIGITAL_COIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // IDC Total
    QString totalIdc = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // IDC
    // Top
    ui->labelAmountTopIdc->setText(totalTransparent);
    ui->labelAmountTopShieldedIdc->setText(totalShielded);
    // Expanded
    ui->labelAmountIdc->setText(totalIdc);
    ui->labelPendingIdc->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureIdc->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
