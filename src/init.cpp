...
void PrepareShutdown()
{
    ...
    util::ThreadRename("islamic_digital_coin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), ISLAMIC_DIGITAL_COIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), ISLAMIC_DIGITAL_COIN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ISLAMIC_DIGITAL_COIN specific functionality (Masternodes, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), ISLAMIC_DIGITAL_COIN_MASTERNODE_CONF_FILENAME));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The ISLAMIC DIGITAL COIN Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("islamic_digital_coin-loadblk");
...
/** Sanity checks
 *  Ensure that ISLAMIC_DIGITAL_COIN is running in a usable environment with all
...
#endif
    LogPrintf("ISLAMIC_DIGITAL_COIN version %s (%s)\n", version_string, CLIENT_DATE);
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single ISLAMIC_DIGITAL_COIN process is using the data directory.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //ISLAMIC_DIGITAL_COIN specific: spork DB's
                ...
                // ISLAMIC_DIGITAL_COIN: load previous sessions sporks if we have them.
