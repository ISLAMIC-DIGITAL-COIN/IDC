...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("ISLAMIC_DIGITAL_COINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("ISLAMIC_DIGITAL_COINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ISLAMIC_DIGITAL_COINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("islamic_digital_coin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ISLAMIC_DIGITAL_COINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ISLAMIC_DIGITAL_COINMiner exception");
    } catch (...) {
        LogPrintf("ISLAMIC_DIGITAL_COINMiner exception");
    }

    LogPrintf("ISLAMIC_DIGITAL_COINMiner exiting\n");
}
