...
CIdcStake* CIdcStake::NewIdcStake(const CTxIn& txin)
{
    ...
    return new CIdcStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CIdcStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CIdcStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CIdcStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CIdcStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CIdcStake::GetUniqueness() const
{
    //The unique identifier for a IDC stake is the outpoint
...
const CBlockIndex* CIdcStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CIdcStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CIdcStake::ContextCheck(int nHeight, uint32_t nTime)
{
