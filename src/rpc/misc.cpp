...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total islamic_digital_coin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in islamic_digital_coin/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in islamic_digital_coin/kb\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"islamic_digital_coinaddress\"\n"
            "\nReturn information about the given islamic_digital_coin address.\n"

            "\nArguments:\n"
            "1. \"islamic_digital_coinaddress\"     (string, required) The islamic_digital_coin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"islamic_digital_coinaddress\",    (string) The islamic_digital_coin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address -only if is standard address-\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for ISLAMIC_DIGITAL_COIN cold staking -only if is standard address-\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ISLAMIC_DIGITAL_COIN address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are islamic_digital_coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) islamic_digital_coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"islamic_digital_coinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"islamic_digital_coinaddress\"  (string, required) The islamic_digital_coin address to use for the signature.\n"
