...
#if defined(NDEBUG)
#error "ISLAMIC_DIGITAL_COIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uidc) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("islamic_digital_coin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> idcInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                idcInputs.push_back(stakeIn);
        }
        const bool hasIDCInputs = !idcInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasIDCInputs) {
                    ...
                    for (const CTxIn& idcIn : idcInputs)
                        if(idcIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasIDCInputs) {
                            for (const CTxIn& stakeIn : idcInputs)
