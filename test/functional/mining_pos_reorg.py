...
class ReorgStakeTest(islamic_digital_coinTestFramework):
    ...
    def check_money_supply(self, expected_idc):
        # verify that nodes have the expected IDC supply
        idc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(idc_supply, [DecimalAmt(expected_idc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that IDC supply was properly updated after the reorgs
        self.log.info("Check IDC supply...")
