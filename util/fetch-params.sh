...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ISLAMIC_DIGITAL_COINParams"
    else
        PARAMS_DIR="$HOME/.islamic_digital_coin-params"
    fi
fi
...
Failed to fetch the ISLAMIC_DIGITAL_COIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
ISLAMIC_DIGITAL_COIN - fetch-params.sh

This script will fetch the ISLAMIC_DIGITAL_COIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ISLAMIC_DIGITAL_COIN zkSNARK parameters. Note that it is
